...
def insert_player_data(row, cursor):
    ...
    INSERT INTO dynastr.sf_player_ranks (
        ...
        fc_sf_rank, fc_one_qb_value, fc_one_qb_rank,
        ktc_sf_normalized_value, fc_sf_normalized_value, ktc_one_qb_normalized_value,
        fc_one_qb_normalized_value,
    ...
    ) VALUES ( %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 'redraft')
...
where 1=1
and (ktc.player_full_name like '%2024 Round%' or ktc.position = 'RDP')
...
inner join (select player_first_name, player_last_name, team, ktc_player_id, sf_value, one_qb_value from dynastr.ktc_player_ranks where rank_type = 'redraft' and rookie = 'false') ktc on concat(p.first_name, p.last_name) = concat(ktc.player_first_name, ktc.player_last_name)
inner join (select sleeper_player_id, sf_value, one_qb_value from dynastr.fc_player_ranks where rank_type = 'redraft') fc on fc.sleeper_player_id = p.player_id 
UNION ALL 
...
from asset_values"""
    ...
    source_columns = ['ktc_sf_value', 'fc_sf_value',]
    normalized_sf_columns = ['ktc_sf_normalized_value',
                             'fc_sf_normalized_value']
    ...
    for source, sf_normalized in zip(source_columns, normalized_sf_columns):
        ...
        values_df[sf_normalized] = (
            (values_df[source] - min_value) / (max_value - min_value)) * 9999

    source_columns = ['ktc_one_qb_value', 'fc_one_qb_value']
    normalized_one_qb_columns = ['ktc_one_qb_normalized_value',
                                 'fc_one_qb_normalized_value']

    for source, one_qb_normalized in zip(source_columns, normalized_one_qb_columns):
        min_value = values_df[source].min()
        max_value = values_df[source].max()
        values_df[one_qb_normalized] = (
            (values_df[source] - min_value) / (max_value - min_value)) * 9999

    values_df['average_normalized_sf_value'] = values_df[normalized_sf_columns].mean(
        axis=1)
    values_df['average_normalized_one_qb_value'] = values_df[normalized_one_qb_columns].mean(
        axis=1)
    sorted_values_df = values_df.sort_values(
        by='average_normalized_sf_value', ascending=False)
    values_df = values_df.fillna(0)
    values_df['sf_harmonic_mean_normalized'] = values_df[normalized_sf_columns].apply(
        hmean, axis=1)
    values_df['one_qb_harmonic_mean_normalized'] = values_df[normalized_one_qb_columns].apply(
        hmean, axis=1)
    sorted_values_df_1 = values_df.sort_values(
        by='sf_harmonic_mean_normalized', ascending=False)
    sorted_values_df_1['sf_harmonic_normalized_rank'] = sorted_values_df_1['sf_harmonic_mean_normalized'].rank(
        ascending=False, method='min')
    sorted_values_df_2 = sorted_values_df_1.sort_values(
        by='one_qb_harmonic_mean_normalized', ascending=False)
    sorted_values_df_2['one_qb_harmonic_normalized_rank'] = sorted_values_df_2['one_qb_harmonic_mean_normalized'].rank(
        ascending=False, method='min')
    df = sorted_values_df_2
    df.rename(columns={'sf_harmonic_mean_normalized': 'superflex_sf_value', 'sf_harmonic_normalized_rank': 'superflex_sf_rank',
              'one_qb_harmonic_mean_normalized': 'superflex_one_qb_value', 'one_qb_harmonic_normalized_rank': 'superflex_one_qb_rank'}, inplace=True)
    ...
    df['superflex_one_qb_value'] = df['superflex_one_qb_value'].fillna(
        0.0).astype(int)
    df['superflex_one_qb_rank'] = df['superflex_one_qb_rank'].fillna(
        0.0).astype(int)
    df['superflex_sf_pos_rank'] = df.groupby('_position')['superflex_sf_value'].rank(
        method='min', ascending=False).fillna(0.0).astype(int)
    df['superflex_one_qb_pos_rank'] = df.groupby('_position')['superflex_one_qb_value'].rank(
        method='min', ascending=False).fillna(0.0).astype(int)
