...
inner join (select player_first_name, player_last_name, team, ktc_player_id, sf_value, one_qb_value from dynastr.ktc_player_ranks where rank_type = 'dynasty' and rookie = 'false') ktc on concat(p.first_name, p.last_name) = concat(ktc.player_first_name, ktc.player_last_name)
...
from asset_values"""
    ...
    normalized_sf_columns = ['ktc_sf_normalized_value',
                             ...
                             'fc_sf_normalized_value', 'dp_sf_normalized_value']
    ...
    conn.commit()
    # Assume you have the SQL query as a string
    sql_query_to_fetch_data = """
    ...
    try:
        for row in data_2025:
            cursor.execute(insert_future_draft_sql, row)
        # Commit the changes
        conn.commit()
